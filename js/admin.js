...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  for (let person of requests) {
    ...
    try {
      row.querySelector(".projects").innerText = person.projects
        .split(", ")
        .map((projectID) => {
  ...
  table.addEventListener("change", (e) => {
    if (e.target.classList.contains("project-select")) {
      ...
      if (
        selectedProject &&
        !projectsCell.innerText.includes(selectedProject)
      ) {
        projectsCell.innerText += projectsCell.innerText
          ? `, ${selectedProject}`
          : selectedProject;
      }
      ...
      if (removeProject && projectsCell.innerText.includes(removeProject)) {
        const projectsArray = projectsCell.innerText
          .split(", ")
          .filter(Boolean);
  ...
  function createNewRow() {
    ...
    for (let room of rooms) {
      ...
      newRow.querySelector(".project-select").innerHTML +=
        `<option value="${roomName}">${roomName}</option>`;
      newRow.querySelector(".project-remove").innerHTML +=
        `<option value="${roomName}">${roomName}</option>`;
    }
  ...
  saveProjectsBtn.addEventListener("click", async () => {
    ...
    const data = Array.from(rows).map((row) => {
      ...
      console.log(
        row.querySelector("#projects").innerHTML,
        getRoomIDbyNames(row.querySelector("#projects").innerHTML),
      );
      const projects = getRoomIDbyNames(
        row.querySelector("#projects").innerHTML,
      );
