...
function displayProjects() {
  ...
  roomList.forEach((roomId) => {
    if (
      ...
      roomId !== "" &&
      roomId !== " " &&
      roomDict[roomId].name
    ) {
      isOwner = true;
      if (
        "editors" in roomDict[roomId] &&
        roomDict[roomId].editors.indexOf(sessionStorage.getItem("email")) != 0
      ) {
        isOwner = false;
      }
...
function selectProject(projectId) {
  ...
  if (project.editors && Array.isArray(project.editors)) {
    project.editors = project.editors.filter(
      (editor) => editor !== sessionStorage.getItem("email"),
    );
...
async function addProject() {
  ...
  try {
    ...
    roomDict[newRoomId] = {
      ...
      name: projectName,
      icon: "default",
      editors: [sessionStorage.getItem("email")],
    };
...
async function ping() {
  ...
  try {
    ...
    const response = await fetch(
      `https://p497lzzlxf.execute-api.us-east-2.amazonaws.com/v1/ping`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
...
function createPopupEffect(container) {
  ...
  for (let i = 0; i < particleCount; i++) {
    particles.push({
      ...
      life: 1,
      color: ["#7c3aed", "#a855f7", "#c084fc", "#e9d5ff"][
      ...
      users.forEach((user) => {
        ...
        userIcon.textContent = (() => {
          ...
          return selectedEmoji
            ? emojiMap[selectedEmoji] || user.emoji || "😊"
            : user.emoji || "😊";
        })();
